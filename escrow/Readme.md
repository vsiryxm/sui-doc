...
源码：https://github.com/MystenLabs/sui/tree/main/examples/trading/contracts/escrow


在 Sui 上，对象可以通过两种方式进行版本控制：快速路径（fastpath）或共识路径（consensus）。这两种路径的选择影响对象的所有权选项，以及应用程序的性能和操作复杂性。许多应用程序可以使用仅使用共识对象或仅使用快速路径对象的解决方案来构建，每种方法都有需要权衡的利弊。
...
在共享对象情况下的协议虽然对称性较弱，但依然是由第一方先锁定他们想要交换的对象开始。

![图5](./images/图5.png)

第二方随后可以查看已被锁定的对象，如果他们决定想要与之交换，就通过创建一个交换请求来表示他们的意愿。
...
这一次，创建请求直接接收被托管的对象（而不是锁定后的对象），并创建一个共享的 Escrow 对象。该请求会记录发送它的地址（如果交换尚未发生，该地址有权取回对象），以及预期的接收方，接收方随后应当通过提供其最初锁定的对象来继续完成交换。
...
### 比较
本节探讨了两种实现两个对象之间交换的方法。在这两种情况下，都会存在这样一个时点：一方已经发出了请求，而另一方尚未回应。此时，双方可能都想访问 Escrow 对象：一方是为了取消交换，另一方是为了完成交换。

* **一种情况**：协议仅使用地址拥有对象，但需要托管人作为中介。它的优点是完全避免了共识带来的成本和延迟，但步骤更多，并且需要信任第三方以保证活性（liveness）。
